...
char * randomIndex()
{
	...
	memset(randomMSG,'\0',sizeof(randomMSG));
	int num;
...
int main() {
	...
	mqd_t mqd_hosttop1, mqd_p1tohost;
	int mq_return = 0;
    ...
    {  
		...
		//	cout<<"already random "<<endl;
			if(mq_send(mqd_hosttop1, (char *)&data, 128, 0) == -1)  
			{  
				cout<<"msgsnd failed"<<endl;
				exit(EXIT_FAILURE);  
			}  
			cout<<"already send"<<data.text<<endl;
